 ...
 * Plateau conforme à ta planche :
 *  - Tour principal = 48 cases, déplacement vers la GAUCHE (antihoraire)
 *  - À +4 : fourche -> on peut continuer ou MONTER (4 cases) -> SOMMET/PORTAIL
 *  - Si on s'arrête sur le SOMMET : au tour suivant on peut se téléporter vers l'un des 3 autres
 *  - Après le sommet (téléporté ou non) : DESCENTE 4 -> retour anneau à +12
 *  - Couloirs d'arrivée au-dessus de la sortie, bases pour les pions non sortis/mangés
 *
 * Règles cartes incluses (version de base) :
 *  - A/K pour sortir (A=1, K=13), 4 recule (horaire), 5 avance partenaire/adversaire
 *  - 7 décomposable simple (7 | 6+1 | 5+2 | 4+3) sur ses propres pions
 *  - Valet = échange deux pions quelconques (avec impact), Joker = n'importe quelle carte
 */
...
function buildDeck(includeJokers = true) {
  ...
  for (const s of SUITS) {
    for (const r of RANKS) deck.push({ id: `${r}${s}-${Math.random()}`, rank: r, suit: s, joker: false });
  }
  if (includeJokers) {
...
// Réglages visuels pour coller à ta planche
const START_INDEXES = [2, 14, 26, 38]; // sorties J/R/B/V (à ajuster si besoin)
const HOME_ENTRY_OFFSET = 1;          // entrée de couloir à +1 (sens gauche) depuis la sortie
const HOME_LEN = 6;                   // longueur du couloir d'arrivée

function modTrack(x) { let v = x % TRACK_LEN; if (v < 0) v += TRACK_LEN; return v; }
function forward(from, steps) { return modTrack(from + DIRECTION * steps); } // avancer = vers la gauche
const TEAM_OF = (p) => (p % 2 === 0 ? 0 : 1); // (0,2) vs (1,3)
const PORTAL_IDS = [0, 1, 2, 3]; // 4 sommets
...
function confForPlayer(i) {
  ...
  return {
    ...
    forkIndex: forward(s, 4),          // +4 : début de montée
    rejoinIndex: forward(s, 12),       // +12 : fin de descente
...
function initialPawns() {
  // 4 pions en base/écurie
  return Array.from({ length: 4 }, () => ({
    location: { kind: "base" }, pieux: false, firstMove: true
  }));
}
...
function posKey(pos) {
  ...
  if (k === "up" || k === "down") return `${k[0].toUpperCase()}:${pos.base}:${pos.step}`; // step 1..4
...
function computePathForward(state, playerIndex, startPos, steps) {
  ...
  for (let i = 0; i < steps; i++) {
    if (cur.kind === "track") {
      ...
      // Entrée de couloir (seulement le sien)
      if (nextIdx === conf.homeEntry) cur = { kind: "home", base: playerIndex, step: 1 };
      else cur = { kind: "track", idx: nextIdx };
    } else if (cur.kind === "home") {
      if (cur.step < conf.homeLen) cur = { kind: "home", base: cur.base, step: cur.step + 1 };
      else return path; // déjà au bout du couloir
    } else if (cur.kind === "up") {
      if (cur.step < 4) cur = { kind: "up", base: cur.base, step: cur.step + 1 };
      else cur = { kind: "portal", base: cur.base };
    } else if (cur.kind === "portal") {
      // avancer depuis portail => descente
      ...
      // base/goal ne se déplacent pas ici
      return [];
    }
    path.push(cur);
  }
...
// Entrer en montée si on est pile sur la fork
  ...
  for (let i = 1; i <= steps; i++) {
    if (cur.kind === "up" && cur.step < 4) {
      ...
      if (cur.step < 4) {
        ...
        const rj = confForPlayer(base).rejoinIndex;
        cur = { kind: "track", idx: rj };
        path.push({ ...cur });
      }
    }
  }
...
function legalMoves(state, playerIndex) {
  ...
  for (const card of player.hand) {
    ...
    for (const rank of ranksToTry) {
      ...
      // Valet = swap de deux pions (impact réel)
      if (rank === "J") {
        const all = [];
        state.players.forEach((pl, pi) =>
          pl.pawns.forEach((pw, pj) => {
            ...
            if (k !== "base" && k !== "goal")
              all.push({ owner: pi, idx: pj, key: posKey(pw.location) });
          })
        );
      ...
      // 5 : avancer partenaire/adversaire (pas soi)
      if (rank === "5") {
        state.players.forEach((pl, pi) => {
      ...
      if (rank === "4") {
        player.pawns.forEach((pw, pj) => {
          ...
          const path = [];
          for (let i = 0; i < 4; i++) {
            cur = { kind: "track", idx: modTrack(cur.idx - DIRECTION * 1) }; // recule (horaire)
            path.push(cur);
          }
      ...
      if (["A","2","3","6","7","8","9","10","Q","K"].includes(rank)) {
        ...
        if (rank === "7") {
          ...
          for (const split of splits) {
            ...
            moves.push(...options.map((pathOps) => ({
              type: "sevenPaths", cardId: card.id, usingRank: rank, pathOps
            })));
          }
        } else {
          player.pawns.forEach((pw, pj) => {
            ...
            // chemin normal
            const path = computePathForward(state, playerIndex, pw.location, steps);
            if (!isBlockedByOccupants(state, { owner: playerIndex, pawnIdx: pj }, path))
              moves.push({ type: "movePath", cardId: card.id, usingRank: rank, owner: playerIndex, pawnIdx: pj, path });

            // possibilité de MONTER si on est pile sur la fork
            const conf = confForPlayer(playerIndex);
...
function enumerateSevenSplits(state, playerIndex, split) {
  ...
  state.players[playerIndex].pawns.forEach((pw, pj) => {
    if (pw.location.kind !== "base" && pw.location.kind !== "goal")
      myRefs.push({ owner: playerIndex, pawnIdx: pj });
  });
  const results = [];
...
function applyMove(state, playerIndex, move) {
  ...
  if (move.type === "swap") {
    ...
    b.location = la;
    a.pieux = b.pieux = false;
    a.firstMove = b.firstMove = false;
  ...
  if (move.type === "movePath") {
    ...
    // sommet -> choix de téléportation au tour suivant
    if (last.kind === "portal") {
    ...
    if (last.kind === "home" && last.step === confForPlayer(owner).homeLen) {
      pawn.location = { kind: "goal", base: owner };
...
export default function App() {
  ...
  function newGame() {
    ...
    const S = {
      ...
      turn: 0, dealer: 0,
      exchangePhase: true, swappedCardBuffer: {},
      pendingPortal: null, log: [], gameOver: false, roundDealCount: 0
    };
    startRound(S);
    return S;
  }
  ...
  function startRound(S) {
    const give = S.roundDealCount === 0 ? 5 : 4; // 1er 5, puis 4
    for (let i = 0; i < 4; i++) {
      const p = S.players[i];
      p.hand = p.hand.concat(S.deck.splice(0, give));
      p.exchangedWithPartner = false;
      p.mustDiscard = false;
    }
    if (S.deck.length === 2) S.discard.push(...S.deck.splice(0)); // 2 cartes en fosse
    S.exchangePhase = true;
    S.roundDealCount += 1;
    S.turn = S.dealer;
  }
  ...
  function checkVictory(S) {
    // TODO: condition de victoire (8 pions rentrés par équipe)
  }

  function applyAndNext(move) {
    setState((prev) => {
      const S = applyMove(prev, prev.turn, move);
      checkVictory(S);
      if (!S.gameOver) nextTurnAfterPlay(S);
      return S;
    });
  }
  ...
  function handleDiscardHand() {
    setState((prev) => {
      ...
      S.discard.push(...pl.hand.splice(0));
      S.log.push(entry(S.turn, "Jette sa main."));
      nextTurnAfterPlay(S);
      return S;
    });
  }
  ...
  function toggleBot(i) {
    setState((prev) => { const S = clone(prev); S.players[i].isBot = !S.players[i].isBot; return S; });
  }

  function exchangeCard(i, cardId) {
    setState((prev) => {
      const S = clone(prev); if (!S.exchangePhase) return S;
      const partner = (i + 2) % 4;
      const hand = S.players[i].hand;
      const idx = hand.findIndex((c) => c.id === cardId);
      if (idx < 0) return S;
      const [card] = hand.splice(idx, 1);
      (S.swappedCardBuffer[partner] ||= []).push(card);
      S.players[i].exchangedWithPartner = true;
      const a = i, b = partner;
  ...
  function usePortal(destPortalId) {
    setState((prev) => {
      const S = clone(prev); if (!S.pendingPortal) return S;
      const { owner, pawnIdx, choices } = S.pendingPortal;
      if (!choices.includes(destPortalId)) return S;
      const pawn = S.players[owner].pawns[pawnIdx];
      pawn.location = { kind: "portal", base: destPortalId }; // se place sur le sommet choisi
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 flex items-center justify-between">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button className="px-3 py-1 rounded-2xl shadow bg-white" onClick={() => setState((s) => ({ ...s }))}>
              {/* noop */}
              Rafraîchir
            </button>
          </div>
...
function Board({ state }) {
  // Dessin conforme : anneau, fourches, portails, montées/desc. visibles, couloirs, bases
  const R_RING = 42;
  const R_PORTAL = 30;
  const R_HOME_BASE = 36;
  const STEP_SHIFT = 3;
  const R_FORK_INNER = 36;
  ...
  return (
    <div className="relative w-full aspect-square bg-gradient-to-br from-amber-100 to-amber-200 rounded-2xl">
        ...
        return (
          <div
            ...
            className={`absolute -translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full border flex items-center justify-center text-[10px]
              ${isFork ? "bg-amber-300 border-amber-700"
                : isHomeEntry ? "bg-emerald-200 border-emerald-700"
                : isStart ? "bg-amber-200 border-amber-600"
      ...
      {/* Branches montée/descente + sommets */}
      {COLORS.map((_, idx) => {
      ...
      {COLORS.map((_, idx) => {
        const conf = confForPlayer(idx);
        const entryAngle = (2 * Math.PI * conf.homeEntry) / TRACK_LEN;
        return [...Array(conf.homeLen)].map((__, stepIdx) => {
          const step = stepIdx + 1;
          const r = R_HOME_BASE - step * STEP_SHIFT;
          const { x: cx, y: cy } = { x: 50 + r * Math.cos(entryAngle), y: 50 + r * Math.sin(entryAngle) };
      ...
      {/* Bases (écuries) – 4 slots près de la sortie */}
      {COLORS.map((c, idx) => {
        const s = START_INDEXES[idx];
        const angle = (2 * Math.PI * s) / TRACK_LEN;
        const r = 32;
        const baseX = 50 + r * Math.cos(angle);
        const baseY = 50 + r * Math.sin(angle);
        const slots = [[-8,-8],[8,-8],[-8,8],[8,8]];
        return (
          <div key={`base-${idx}`} className="absolute" style={{ left: `${baseX}%`, top: `${baseY}%`, transform: "translate(-50%, -50%)" }}>
            <div className="w-16 h-16 rounded-xl border-2 border-neutral-600 flex items-center justify-center" style={{ background: "#fff8" }}>
              <div className="relative w-14 h-14">
                {slots.map((ofs, i2) => {
                  const occ = firstBasePawn(state, idx);
                  return (
                    <div
                      key={i2}
...
function firstBasePawn(state, ownerIndex) {
  // sert juste à colorer un slot si au moins 1 pion est en base
  const p = state.players[ownerIndex].pawns.find((x) => x.location.kind === "base");
  return p || null;
}

function PawnDot({ color }) {
  return <div className="w-3 h-3 rounded-full" style={{ background: colorToken(color) }} />;
}
...
function TurnPanel({ state, moves, onPlay, onDiscard }) {
  return (
    <div className="bg-white rounded-2xl shadow p-3">
      ...
      {state.exchangePhase ? (
        <div className="text-sm opacity-70">Phase d'échange : donne 1 carte à ton partenaire.</div>
      ) : (
        <>
          <div className="text-sm mb-2">Coups légaux: {moves.length}</div>
          <div className="flex flex-wrap gap-2">
            {moves.slice(0, 24).map((m, i) => (
...
function ExchangePanel({ state, onChoose }) {
  ...
  if (me.isBot) return <div className="text-sm opacity-60">Le bot choisit une carte à donner...</div>;
  return (
    <div>
      <div className="mb-2">Choisis <strong>1 carte</strong> à donner à ton partenaire ({["J1","J2","J3","J4"][partner]}).</div>
      <div className="flex gap-2 flex-wrap">
        {me.hand.map((c) => (
